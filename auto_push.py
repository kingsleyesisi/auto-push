...
GITHUB_EMAIL = os.getenv('USERMAIL')
if not GITHUB_USERNAME or not GITHUB_EMAIL:
    raise RuntimeError("❌ GITHUB_USERNAME or USERMAIL not set")
...
def configure_git_identity():
    run_cmd(["git", "config", "--global", "user.name",  GITHUB_USERNAME]) # Change to your user name
    run_cmd(["git", "config", "--global", "user.email", GITHUB_EMAIL]) # Change to your github email
...
def commit_and_push():
    ...
    msg = f"{COMMIT_PREFIX} {datetime.now().strftime('%Y-%m-%d %H:%M:%SZ')}"
